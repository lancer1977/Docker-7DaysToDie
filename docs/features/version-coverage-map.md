...
| --- | --- | --- | --- | --- |
| `V0` infra baseline | `established` | Runtime image source, LinuxGSM install path, ports, volumes, mods/backups/monitoring scripts, deployment docs, and runtime contract smoke exist | `Dockerfile`, `install.sh`, `user.sh`, `scripts/`, docs, `scripts/validate.sh`, `scripts/smoke_runtime_contract.sh` | Run the full image build and container smoke when changing runtime behavior |
...
ports, volume mounts, deployment docs, and a source-level runtime contract smoke
for entrypoint and start-mode routing.
